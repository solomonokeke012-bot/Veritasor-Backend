...
export interface PasswordValidationResult {
  ...
  errors: string[];
  warnings: string[];
  strengthScore: number; // 0-100, higher = stronger
}
 ...
 */
export const DEFAULT_ABUSE_PREVENTION_CONFIG: AbusePreventionConfig = {
  minPasswordLength: 8,
...
export function validateEmail(
  email: string,
  config: AbusePreventionConfig = DEFAULT_ABUSE_PREVENTION_CONFIG,
 ...
 */
export function looksLikeRandomString(str: string): boolean {
 ...
 */
export function hasRepeatingPattern(str: string): boolean {
...
export function validatePassword(
  ...
  if (password.length > config.maxPasswordLength) {
    errors.push(
      `Password must not exceed ${config.maxPasswordLength} characters`,
    );
  }
 ...
 */
export function hasSequentialChars(str: string): boolean {
  const lower = str.toLowerCase();
 ...
 */
export function hasKeyboardPattern(str: string): boolean {
  const lower = str.toLowerCase();
...
export function calculateBackoffDelay(
  ...
  const delay = Math.min(
    baseDelayMs * Math.pow(2, failedAttempts - 1),
    maxDelayMs,
  );
 ...
 */
export function getDisposableEmailStats(): { domainCount: number } {
  return {
 ...
 */
export function getWeakPasswordStats(): { passwordCount: number } {
  return {

...
} from "../../utils/signupRateLimiter.js";
...
/**
 * Response payload for successful signup
 */
...
/**
 * Error types for signup failures
 */
...
export class SignupError extends Error {
  public readonly type: SignupErrorType;
...
/**
 * Configuration for signup service
 */
export interface SignupServiceConfig {
  /** Abuse prevention configuration */
  abusePrevention: AbusePreventionConfig;
  /** Rate limiting configuration */
  rateLimit: Partial<SignupRateLimitConfig>;
  /** Minimum time for signup operation (ms) - prevents timing attacks */
  minOperationTimeMs: number;
  /** Whether to enable honeypot detection */
  enableHoneypot: boolean;
  /** Whether to enable suspicious activity warnings */
  enableSuspiciousActivityLogging: boolean;
}
...
export async function signup(
  request: SignupRequest,
  config: Partial<SignupServiceConfig> = {},
): Promise<SignupResponse> {
  const startTime = Date.now();
  const fullConfig: SignupServiceConfig = {
  ...
  if (!validation.valid) {
    ...
    rateLimiter.recordFailure(
      clientIp,
      validation.normalizedEmail || normalizeEmail(request.email),
    );
  ...
  if (!rateLimitCheck.allowed) {
    ...
    throw new SignupError(
      rateLimitCheck.blockReason ||
        "Too many signup attempts. Please try again later.",
      "RATE_LIMITED",
      429,
      [rateLimitCheck.blockReason || "Rate limit exceeded"],
    );
  }

    ...
    try {
        ...
        if (result.rows.length === 0) {
            if (expectedVersion !== undefined) {
                const existing = await getById(client, id);
                if (existing) {
                    throw createConflictError(ConflictErrorType.CONFLICT_TYPE_VERSION, `Attestation ${id} has been modified by another process`, { id, expectedVersion, currentVersion: existing.version });
                }
            }
            return null;
        }

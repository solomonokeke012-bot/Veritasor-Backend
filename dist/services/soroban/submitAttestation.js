import { BASE_FEE, Contract, Keypair, StrKey, TransactionBuilder, nativeToScVal, scValToNative } from '@stellar/stellar-sdk';
...
function mapSendResponseError(response) {
 ...
 */
export function validateSendTransactionResponse(response) {
    if (!response || typeof response !== 'object') {
        throw new SorobanSubmissionError('sendTransaction returned an invalid response object.', 'INVALID_RESPONSE', response);
    }
    if (typeof response.hash !== 'string' || !TX_HASH_RE.test(response.hash)) {
        throw new SorobanSubmissionError(`sendTransaction returned an invalid transaction hash: "${response.hash}".`, 'INVALID_RESPONSE', response);
    }
    const validStatuses = ['PENDING', 'DUPLICATE', 'ERROR', 'TRY_AGAIN_LATER'];
    if (!validStatuses.includes(response.status)) {
        throw new SorobanSubmissionError(`sendTransaction returned an unexpected status: "${response.status}".`, 'INVALID_RESPONSE', response);
    }
}
/**
 ...
 */
export async function waitForConfirmation(server, txHash, pollIntervalMs = CONFIRMATION_POLL_INTERVAL_MS, maxAttempts = CONFIRMATION_MAX_ATTEMPTS) {
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        if (txResponse.status === 'FAILED') {
            throw new SorobanSubmissionError('Transaction was included in a ledger but execution failed.', 'CONFIRMATION_FAILED', txResponse);
        }
    ...
    throw new SorobanSubmissionError(`Transaction ${txHash} was not confirmed after ${maxAttempts} polling attempts.`, 'CONFIRMATION_TIMEOUT');
}
/**
 ...
 */
export function validateConfirmedResult(txResponse, submittedMerkleRoot) {
    if (!txResponse.returnValue) {
        throw new SorobanSubmissionError('Confirmed transaction has no return value. The contract may not have returned attestation data.', 'RESULT_VALIDATION_FAILED', txResponse);
    }
    let native;
    try {
        native = scValToNative(txResponse.returnValue);
    }
    catch (err) {
        throw new SorobanSubmissionError('Failed to decode the contract return value from the confirmed transaction.', 'RESULT_VALIDATION_FAILED', err);
    }
    if (!native || typeof native !== 'object') {
        throw new SorobanSubmissionError('Contract return value is not a valid object.', 'RESULT_VALIDATION_FAILED', native);
    }
    const onChainRoot = typeof native.merkle_root === 'string'
        ? native.merkle_root
        : String(native.merkle_root ?? '');
    if (!onChainRoot) {
        throw new SorobanSubmissionError('Confirmed transaction result does not contain a merkle_root field.', 'RESULT_VALIDATION_FAILED', native);
    }
    if (onChainRoot !== submittedMerkleRoot) {
        throw new SorobanSubmissionError(`On-chain merkle root "${onChainRoot}" does not match submitted value "${submittedMerkleRoot}".`, 'RESULT_MISMATCH', { expected: submittedMerkleRoot, actual: onChainRoot });
    }
    ...
    if (onChainTimestamp === undefined || !Number.isFinite(onChainTimestamp)) {
        throw new SorobanSubmissionError('Confirmed transaction result does not contain a valid timestamp.', 'RESULT_VALIDATION_FAILED', native);
    }
...
export async function submitAttestation(params) {
    ...
    try {
        ...
        const operation = contract.call('submit_attestation', nativeToScVal(params.business), nativeToScVal(params.period), nativeToScVal(params.merkleRoot), nativeToScVal(normalizeTimestamp(params.timestamp), { type: 'u64' }), nativeToScVal(params.version));
        ...
        if (!signerSecret) {
            throw new SorobanSubmissionError('No signer secret available. Provide params.signerSecret or set SOROBAN_SOURCE_SECRET, or call with submit:false.', 'MISSING_SIGNER');
        }
        const signer = Keypair.fromSecret(signerSecret);
        if (signer.publicKey() !== params.sourcePublicKey) {
            throw new SorobanSubmissionError('signerSecret does not match sourcePublicKey.', 'SIGNER_MISMATCH');
        }
        ...
        try {
            ...
            const successResponse = confirmed;
            ...
            if (confirmError instanceof SorobanSubmissionError &&
        ...
        throw new SorobanSubmissionError('Failed to build or submit attestation transaction on Soroban.', 'SOROBAN_NETWORK_ERROR', error);
    }
}
